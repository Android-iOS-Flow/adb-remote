...
export async function loadDevices(silent) {
  ...
  if (!token) { if (!silent) alert(t("conn.enterTokenFirst")); return []; }
  try {
    const res = await fetch(`/api/devices?token=${encodeURIComponent(token)}`);
    if (!res.ok) { if (!silent) alert(t("conn.wrongToken")); return []; }
    const list = await res.json();
    const sel = $("devices");
    const prev = sel.value;
    sel.innerHTML = "";
    if (list.length === 0) sel.innerHTML = `<option value="">${t("conn.noDevices")}</option>`;
    for (const d of list) {
    ...
    if (!silent) alert(t("conn.loadFailed", { err: e }));
    return [];
  }
}
